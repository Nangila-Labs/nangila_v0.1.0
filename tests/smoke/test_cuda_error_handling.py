...
CUDA_AVAILABLE = NANGILA_AVAILABLE and nangila.cuda_available()


def experimental_api():
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        return nangila.experimental
...
class TestCudaErrorHandling:
    ...
    def test_cuda_functions_raise_without_cuda(self):
        ...
        with pytest.raises(RuntimeError, match="CUDA not compiled"):
            experimental_api().cuda_predict_and_quantize(
                0, 0, 0, 0.9, 0.1, 0, 100, 0, SyncMode.ASYNC
            )
        
        with pytest.raises(RuntimeError, match="CUDA not compiled"):
            experimental_api().cuda_dequantize_and_reconstruct(
                0, 0, 0, 0.9, 0.1, 0, 100, 0, SyncMode.ASYNC
            )
