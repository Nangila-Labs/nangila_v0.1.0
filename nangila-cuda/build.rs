...
fn main() {
    ...
    if !cuda_include.exists() {
        println!("cargo:warning=CUDA not found at {}. Using CPU fallback.", cuda_path);
    ...
    for cu_file in &cu_files {
        ...
        match status {
            ...
            Ok(s) => {
                println!("cargo:warning=nvcc failed for {} with exit code {:?}", cu_file, s.code());
            }
            Err(e) => {
                println!("cargo:warning=Failed to run nvcc: {}. Using CPU fallback.", e);
                println!("cargo:rustc-cfg=feature=\"cpu_fallback\"");
                return;
            }
        }
